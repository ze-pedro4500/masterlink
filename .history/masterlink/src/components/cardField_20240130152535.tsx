  ...
  return (
    <div>
      {cardData.map((card, index) => (
        <div key={index}>
          <a className='mb-10' href={card.link} target="_blank" rel="noopener noreferrer"><img className='max-h-14' title={card.text} height="40" src={card.imageUri} alt={card.text} />
          </a>
        </div>
      ))}
    </div>
