    ...
    <div>
      {cardData.map((card, index) => (
        <div className='mb-4' key={index}>
          <a href={card.link} target="_blank" rel="noopener noreferrer"><img className='mx-auto max-h-14 max-w-14' title={card.text} height="40" src={card.imageUri} alt={card.text} />
          </a>
        </div>
      ))}
    </div>
